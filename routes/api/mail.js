...
router.post("/reset-password/student", async (req, res) => {
  ...
  try {
    // find the email
    const student = await Student.findOne({ email: emailAddress });

    if (!student) {
      return res.status(400).json({ msg: "Please check the mail!" });
    }
    ...
    let info = await transporter.sendMail({
      ...
      html:
        "<p>Your password has been reset to password given below:</p><br>" +
        "password: <b>" +
        genPassword +
        "</b><br>" +
    ...
    if (info.messageId) {
      ...
      return res.status(500).json({ msg: "Could not send email" });
    }
  } catch (err) {
...
router.post("/reset-password/admin", async (req, res) => {
  ...
  try {
    // find the email
    const admin = await Admin.findOne({ email: emailAddress });
